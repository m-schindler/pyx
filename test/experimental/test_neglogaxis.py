...
def plot_xy_function(xmin=0.72, xmax=7.85):
    ...
    yparter = graph.axis.parter.logarithmic(
      tickpreexps=[graph.axis.parter.logarithmic.pre1exp, graph.axis.parter.logarithmic.pre1to9exp],
      extendtick=0, extendlabel=None
    )
    yrater = graph.axis.rater.logarithmic()
    yaxis = graph.axis.logarithmic(title=r"$y$", min=None, max=None, parter=yparter, rater=yrater)
