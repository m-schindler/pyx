...
class textextbox_pt(textbox_pt):
    ...
    def __init__(self, x_pt, y_pt, left_pt, right_pt, height_pt, depth_pt, do_finish, fontmap, singlecharmode, fillstyles, convert_to_path):
        """Text output.
        ...
        self.convert_to_path = convert_to_path
    ...
    def processPS(self, file, writer, context, registry, bbox):
        ...
        if self.convert_to_path:
            c = canvas.canvas()
            c.fill(self.textpath())
            c.processPS(file, writer, context, registry, abbox)
            bbox += c.bbox()
        else:
    ...
    def processPDF(self, file, writer, context, registry, bbox):
        ...
        if self.convert_to_path:
            c = canvas.canvas()
            c.fill(self.textpath())
            c.processPDF(file, writer, context, registry, abbox)
            bbox += c.bbox()
        else:
    ...
    def processSVG(self, xml, writer, context, registry, bbox):
        ...
        if self.convert_to_path:
            c = canvas.canvas()
            c.fill(self.textpath())
            c.processSVG(file, writer, context, registry, abbox)
            bbox += c.bbox()
        else:
...
class SingleEngine:
    ...
    def text_pt(self, x_pt, y_pt, expr, textattrs=[], texmessages=[], fontmap=None, singlecharmode=False, convert_to_path=None):
        """Typeset text.
        ...
        if convert_to_path is None:
            convert_to_path = self.convert_to_path
        box = textextbox_pt(x_pt, y_pt, left_pt, right_pt, height_pt, depth_pt, self.do_finish, fontmap, singlecharmode, fillstyles, convert_to_path)
        for t in trafos:
